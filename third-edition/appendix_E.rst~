...
``SHORT_DATETIME_FORMAT``, or a custom format that uses the format
...
attached timezone information (a timezone-aware ``datetime`` instance) the ``time`` filter will
...
internationalization  in templates. They allow for
...
template with the ``{% load %}<load>`` tag.
...
such as a generator of Lorem Ipsum text.
...
with a ``static`` template tag. You can use this regardless if you're
...
.. note::
    ...
    with a ``static template tag<staticfiles-static>`` which uses
    ``staticfiles'`` ``STATICFILES_STORAGE`` to build the URL of the
    given path (rather than simply using :func:`urllib.parse.urljoin` with the
    ``STATIC_URL`` setting and the given path). Use that instead if you
    have an advanced use case such as using a cloud service to serve
    static files::
