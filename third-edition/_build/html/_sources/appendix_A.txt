...
Chapter 4 explains the basics of defining models, and we use them throughout
...
:class:`~django.core.files.File` class reference and the Files
topic guide.
...
See the Django documentation for other caveats.
...
:class:`~django.forms.ModelForm`. They are also skipped during model
...
portion of the field will be considered. Besides, when ``USE_TZ`` is ``True``,
...
:class:`~django.forms.ModelForm` (for example, if one of the fields is listed
in ``exclude`` or has :attr:`editable=False<Field.editable>`),
...
Every ``Field`` instance contains several attributes that allow introspecting
its behavior. Use these attributes instead of ``isinstance`` checks when you
...
specific field types.  Custom model fields should implement these flags.
...
:class:`ForeignKey`, including recursive and
lazy relationships.
...
.. admonition:: Use lowercase table names for MySQL
    ...
    backend. See Chapter 23 for more details.
...
.. attribute:: Options.default_permissions
    ...
    the model is created by ``migrate`` in order to prevent any omitted
